...
# 365 % 7 = 1
UKEDAGER = ["søn","man","tir","ons","tor","fre","lør"]
...
for i in range(1900,1920):
    print(f"{i} {UKEDAGER[weekday_newyear(i)%7]}")
...
def workdays_in_year(year):
    dager = 0
    dager_i_året = 365
    første_dag = weekday_newyear(year)
    if is_leap_year(year): 
        dager_i_året = 366
    for i in range(første_dag,første_dag+dager_i_året):
        dager += 1*(is_workday(i))
    return dager
