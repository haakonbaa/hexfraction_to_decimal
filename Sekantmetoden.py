...
def secant_method(x0, x1, func, tol):
    x_forrige = x0  #forrige
    x_nå = x1       #nå
    while abs(func(x_nå)) > tol:
        x_neste = x_nå - func(x_nå) / differentiate(x_nå,x_forrige,func)
        x_forrige = x_nå
        x_nå = x_neste
    print(f"Funksjonen nærmer seg et nullpunkt når x = {x_nå:.4f}, da er f(x) = {func(x_nå):.2e}")
